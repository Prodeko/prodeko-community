...
ifeq ($(ENV),prod)
	...
	SEED = cat ./directus/seed.sql | docker-compose exec -T talks_database psql -U ${DB_USER} -d ${DB_DATABASE}
	WAIT = $(COMPOSE) run wait -c ${DB_HOST}:${DB_PORT}
	DATABASE = $(COMPOSE) up -d talks_database
endif
